...
Respect pipeline phase ownership: a requirement solely about this run's later Push/PR/CI is deferred to that step, not a contradiction before it runs. A decision recorded at Test whose remaining requirement can only be proven by driving the running product live is deferred the same way, to Test's own next round, since this review phase cannot run the product. Assess any source-verifiable portion now and explain the deferred delivery in evidence; external or pre-existing lifecycle requirements remain enforceable. When the only thing left to check is that live drive, and nothing in the current tree contradicts the decision, return satisfied with evidence naming the deferred live proof rather than unverified, so the decision does not park here only to be re-proven live moments later by Test itself; return unverified only when something besides that deferred live proof is genuinely unknown.
