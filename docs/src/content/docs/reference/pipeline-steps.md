...
- Asks the agent to find documentation gaps, update documentation files, verify its edits, and commit changes under the placement policy. Source or doc-comment edits (including agent-created commits) are preserved but stop the step for review and validation in a new run.
...
When `commands.lint` is empty, unresolved findings from the combined pass pause for approval instead of starting another automatic lint/fix loop, because housekeeping reports lint issues rather than fixing them.
...
- Updating any existing PR preserves its live title, even when [`pr.title_format`](/no-mistakes/reference/repo-config/#prtitle_format) is configured. An unowned body is still redrafted on every update.
