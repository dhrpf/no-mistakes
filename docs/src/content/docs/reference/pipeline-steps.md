...
- Updating an existing, unowned-body (non-templated) PR preserves its live title: the pipeline never sends `--title` unless [`pr.title_format`](/no-mistakes/reference/repo-config/#prtitle_format) is configured, in which case the format is redrafted and applied on every managed update as documented there. The body is still redrafted on every update regardless.
