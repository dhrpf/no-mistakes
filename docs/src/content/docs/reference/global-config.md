...
For empty `commands.lint`, the document step's combined housekeeping pass also detects and reports lint issues without fixing them, and the lint step consumes its result; unresolved blocking lint findings then pause for approval instead of starting another automatic fix loop.
